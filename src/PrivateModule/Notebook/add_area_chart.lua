...
PrivateModuleObject.Notebook___add_area_chart = function(selfobj, data, options)
    ...
    if #series_names > 1 then
        ...
        for si, name in ipairs(series_names) do
            ...
            p[#p+1] = string.format('<rect x="%g" y="%g" width="14" height="10" fill="%s" fill-opacity="0.4" rx="2"/>', lx, ty - 9, color)
    ...
    -- Baseline y (y=0 in data coordinates)
    local baseline_y = val_to_y(0)
    ...
    -- Areas (filled polygons) — drawn before lines so lines appear on top
    for si, name in ipairs(series_names) do
        local color = colors[name] or PALETTE[((si - 1) % NUM_PALETTE) + 1]
        local vals  = series[name]
        local pts   = {}
        -- Start at baseline of first point
        table.insert(pts, string.format("%g,%g", 0, baseline_y))
        for i, v in ipairs(vals) do
            table.insert(pts, string.format("%g,%g", (i - 1) * slot, val_to_y(v)))
        end
        -- Close back down to baseline at last point
        table.insert(pts, string.format("%g,%g", (n - 1) * slot, baseline_y))

        p[#p+1] = string.format(
            '<polygon points="%s" fill="%s" fill-opacity="0.2"/>',
            table.concat(pts, " "), color)
    end

    -- Lines and dots — drawn on top of areas
    for si, name in ipairs(series_names) do
        local color = colors[name] or PALETTE[((si - 1) % NUM_PALETTE) + 1]
        local vals  = series[name]
        local pts   = {}
